...
def show_chat_page(cookies):
    ...
    # ---------------- EMPTY STATE ----------------
    if st.session_state.conversation_id is None:
        st.info("Start a new conversation by asking a question about your study materials.")
    ...
    if user_input:
        ...
        with st.chat_message("assistant"):
            with st.spinner("Thinking... 📚"):
                ...
                # Extract the final string output from the agent
                answer = result.get("output", "I encountered an error generating a response.")
